...
class _RepetitionMixin:
    ...
    def __init__(self, s, spacing):
        ...
        self.s = s
        self.spacing = util.Vector(
            *(float("inf") if x is None or x == 0 else x for x in spacing)
        )
    ...
    def bounding_box(self):
        ...
        return util.BoundingBox(
            util.Vector(
                *(x if s is None else -float("inf") for x, s in zip(b.a, self.spacing))
            ),
            util.Vector(
                *(x if s is None else float("inf") for x, s in zip(b.b, self.spacing))
            ),
        )
    ...
    def get_node(self, point, cache):
        return self.s.get_node(
            cache.make_node("repetition", self.spacing, [point]), cache
        )
...
class _CircularRepetitionMixin:
    ...
    def get_node(self, point, cache):
        ...
        new_point = cache.make_node("circular_repetition_to", [pi_over_n], [point])
        sub_node = self.s.get_node(new_point, cache)
        return cache.make_node(
            "circular_repetition_from", [pi_over_n], [sub_node, point]
        )
