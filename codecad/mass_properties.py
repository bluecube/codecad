...
LOCATION_QUEUE_SIZE = (
    2 ** 26
)  # More is faster, but we want even low end gpus to not have any problems
# TODO: Change this value based on gpu

assert (
    TREE_CHILD_COUNT < 32
), "Set of node children must be representable by uint bitmask"
assert (
    TREE_CHILD_COUNT * MAX_WORK_SIZE <= 2 ** 23
), "Maximum number of split nodes per run must be exactly representable as float"
...
class MassProperties(
    collections.namedtuple(
        "MassProperties", "volume centroid inertia_tensor volume_error"
    )
):
...
def mass_properties(
    shape,
    abs_allowed_error=None,
    rel_allowed_error=1e-4,
    ...
    with cl_util.BufferList() as buffer_list:
        ...
        locations = cl_util.Buffer(
            cltypes.float4, LOCATION_QUEUE_SIZE, pyopencl.mem_flags.READ_WRITE
        )
        buffer_list.add(locations)
        allowed_errors = cl_util.Buffer(
            cltypes.float, LOCATION_QUEUE_SIZE, pyopencl.mem_flags.READ_WRITE
        )
        buffer_list.add(allowed_errors)
        temp_locations = cl_util.Buffer(
            cltypes.float4, MAX_WORK_SIZE, pyopencl.mem_flags.READ_WRITE
        )
        buffer_list.add(temp_locations)
        temp_allowed_errors = cl_util.Buffer(
            cltypes.float, MAX_WORK_SIZE, pyopencl.mem_flags.READ_WRITE
        )
        buffer_list.add(temp_allowed_errors)
        integral1 = cl_util.Buffer(
            cltypes.float4, MAX_WORK_SIZE, pyopencl.mem_flags.READ_WRITE
        )
        buffer_list.add(integral1)
        integral2 = cl_util.Buffer(
            cltypes.float4, MAX_WORK_SIZE, pyopencl.mem_flags.READ_WRITE
        )
        buffer_list.add(integral2)
        integral3 = cl_util.Buffer(
            cltypes.float4, MAX_WORK_SIZE, pyopencl.mem_flags.READ_WRITE
        )
        buffer_list.add(integral3)
        split_counts = cl_util.Buffer(
            cltypes.uint, MAX_WORK_SIZE, pyopencl.mem_flags.READ_WRITE
        )
        buffer_list.add(split_counts)
        split_masks = cl_util.Buffer(
            cltypes.uint, MAX_WORK_SIZE, pyopencl.mem_flags.READ_WRITE
        )
        buffer_list.add(split_masks)
        output_buffer = cl_util.Buffer(
            cltypes.float,
            13,
        ...
        prepare_next_ev = locations.enqueue_write(
            box.a.as_float4(initial_step_size), wait_for=[ev]
        )
        first_location = 0
        location_count = 1
        ...
        dfs_switch_threshold = LOCATION_QUEUE_SIZE - math.ceil(
            math.log(2 ** 23, TREE_SIZE)
        ) * MAX_WORK_SIZE * (TREE_CHILD_COUNT - 1)
        ...
        while location_count > 0:
            ...
            if location_count <= dfs_switch_threshold:
                ...
                work_size_limit = (LOCATION_QUEUE_SIZE - location_count) // (
                    TREE_CHILD_COUNT - 1
                )
                if work_size_limit == 0:
                    raise Exception(
                        "Maximum location buffer size exceeded. "
                    ...
                    logger.warning(
                        "Limiting work size to %i to avoid exceeding location buffer size. "
                        "This is likely a bug in CodeCad, but should only affect performance.",
                        work_size,
                    )

                start_offset = (
                    first_location + location_count - work_size
                ) % LOCATION_QUEUE_SIZE
            ...
            next_allowed_error = max(
                abs_allowed_error, integral_one.result * rel_allowed_error
            )
            ...
            evaluate_ev = opencl_manager.k.mass_properties_evaluate(
                [work_size],
                None,
                program_buffer,
            ...
            sum_ev = opencl_manager.k.mass_properties_sum(
                [work_size],
                None,
                integral1,
                integral2,
            ...
            output_ev = opencl_manager.k.mass_properties_output(
                [1],
                None,
            ...
            first_location = (
                first_location + first_location_move
            ) % LOCATION_QUEUE_SIZE

            prepare_next_ev = opencl_manager.k.mass_properties_prepare_next(
                [work_size],
                None,
            ...
            with output_buffer.map(
                pyopencl.map_flags.READ, wait_for=[output_ev]
            ) as mapped:
            ...
            time_computing += (
                cl_util.event_time_spent(evaluate_ev)
                + cl_util.event_time_spent(sum_ev)
                + cl_util.event_time_spent(output_ev)
            )
            ...
            logger.debug(
                "Iteration finished: %.4f%%, total error used: %e, location count: %i, "
                ...
                "%s mode, bonus allowed error: %e, split count: %i (%.1f / location)",
                100 * progress,
                total_error.result,
                location_count,
                time_spent,
                time_computing.result,
                time_computing.result / time_spent,
                iteration_count,
                iteration_count / time_spent,
                locations_processed,
                util.safe_div(locations_processed, iteration_count),
                locations_processed / time_spent,
                "BFS" if bfs_mode else "DFS",
                allowed_error_per_volume,
                splits,
                splits / work_size,
            )
    ...
    logger.info(
        ...
        time_computing.result,
        time_computing.result / time_spent,
        util.safe_div(time_summing.result, time_computing.result),
        iteration_count,
        iteration_count / time_spent,
        locations_processed,
        util.safe_div(locations_processed, iteration_count),
    ...
    if volume == 0:
        return MassProperties(
            0, util.Vector.splat(0), numpy.zeros((3, 3)), total_error.result
        )
