...
@pytest.mark.parametrize(
    "points",
    [pytest.param(v, id=k) for k, v in sorted(data.valid_polygon2d.items())]
    + [
        pytest.param(list(reversed(v)), id="reversed_" + k)
        for k, v in sorted(data.valid_polygon2d.items())
    ],
)
def test_valid_polygon_construction(points):
    codecad.shapes.simple2d.Polygon2D(
        points
...
@pytest.mark.parametrize(
    "points",
    [pytest.param(v, id=k) for k, v in sorted(data.invalid_polygon2d.items())]
    + [
        pytest.param(list(reversed(v)), id="reversed_" + k)
        for k, v in sorted(data.invalid_polygon2d.items())
    ],
)
...
def test_regular_polygon2d_hexagon():
    ...
    assert codecad.shapes.simple2d.RegularPolygon2D.calculate_n(1, 1) == pytest.approx(
        6
    )
...
def test_square_feature_size():
    assert (
        codecad.shapes.rectangle(1, 1).feature_size()
        == pytest.approx(
            codecad.shapes.regular_polygon2d(4, side_length=1).feature_size()
        )
        == 1
    )
...
def test_regular_polygon_feature_size(n):
    points = [
        (math.cos(math.pi * 2 * (i + 0.5) / n), math.sin(math.pi * 2 * (i + 0.5) / n))
        for i in range(n)
    ]

    assert codecad.shapes.regular_polygon2d(n, r=1).feature_size() == pytest.approx(
        codecad.shapes.polygon2d(points).feature_size()
    )
