...
def test_buffer_indexing(size):
    b = codecad.cl_util.Buffer(
        pyopencl.cltypes.uint3, size, pyopencl.mem_flags.WRITE_ONLY
    )
...
@pytest.mark.parametrize(
    "item_type, item_size",
    [
        (pyopencl.cltypes.uint3, 16),
        (pyopencl.cltypes.uchar, 1),
        (pyopencl.cltypes.double16, 128),
    ],
)
def test_buffer_alloc_size(size, nitems, item_type, item_size):
    b = codecad.cl_util.Buffer(item_type, size, pyopencl.mem_flags.WRITE_ONLY)

    b.create_host_side_array()

    assert b.nitems == nitems
    assert b.size == nitems * item_size  # noqa
    assert b.array.nbytes == b.size  # noqa
...
def test_buffer_write_map():
    b = codecad.cl_util.Buffer(
        pyopencl.cltypes.uchar8, 1, pyopencl.mem_flags.READ_WRITE
    )
...
@pytest.mark.parametrize(
    "item_type",
    [(pyopencl.cltypes.uint3), (pyopencl.cltypes.uchar), (pyopencl.cltypes.double16)],
)
def test_buffer_map_size(item_type):
    b = codecad.cl_util.Buffer(item_type, (2, 2, 2), pyopencl.mem_flags.WRITE_ONLY)

    with b.map(pyopencl.map_flags.WRITE_INVALIDATE_REGION, wait_for=None) as mapped:
        assert mapped.nbytes == b.size  # noqa
...
def test_assert_pass():
    ...
    ev = codecad.cl_util.opencl_manager.k.assert_tester(
        (10, 10), None, numpy.uint32(100), assert_buffer
    )
...
def test_assert_fail():
    assert_buffer = codecad.cl_util.AssertBuffer()
    ev = codecad.cl_util.opencl_manager.k.assert_tester(
        (10, 10), None, numpy.uint32(5), assert_buffer
    )
...
def test_assert_fail_multiple():
    assert_buffer = codecad.cl_util.AssertBuffer()
    ev = codecad.cl_util.opencl_manager.k.assert_tester(
        (10, 10, 1000), None, numpy.uint32(5), assert_buffer
    )
...
def test_assert_chaining_pass():
    assert_buffer = codecad.cl_util.AssertBuffer()
    ev = codecad.cl_util.opencl_manager.k.assert_tester(
        (10, 10), None, numpy.uint32(101), assert_buffer
    )
    ev = codecad.cl_util.opencl_manager.k.assert_tester(
        (10, 10), None, numpy.uint32(102), assert_buffer, wait_for=[ev]
    )
...
def test_assert_chaining_multiple_fail():
    assert_buffer = codecad.cl_util.AssertBuffer()
    ev = codecad.cl_util.opencl_manager.k.assert_tester(
        (10, 10), None, numpy.uint32(1), assert_buffer
    )
    ev = codecad.cl_util.opencl_manager.k.assert_tester(
        (10, 10), None, numpy.uint32(2), assert_buffer, wait_for=[ev]
    )
    ...
    for job, step in job_log:
        ...
        if job > 3:
            assert (
                job // 2,
                2,
            ) in checked, "Parent task must be finished before running a child"
...
def test_format_c_string_literal(string):
    ...
    with warnings.catch_warnings():
        ...
        program = pyopencl.Program(
            codecad.cl_util.opencl_manager.context,
            """

__kernel void copy_string(__global char* output, uint n)
{
    __constant const char* p = """
            + codecad.cl_util.format_c_string_literal(string)
            + """;
    for (uint i = 0; i < n; ++i)
        *output++ = *p++;
}""",
    ...
    output = codecad.cl_util.Buffer(
        pyopencl.cltypes.uchar, 256, pyopencl.mem_flags.WRITE_ONLY
    )

    ev = program.copy_string(
        codecad.cl_util.opencl_manager.queue,
        (1,),
        None,
        output,
        numpy.uint32(len(encoded) + 1),
    )
...
def test_sum(step_size):
    buffer1 = codecad.cl_util.Buffer(
        pyopencl.cltypes.float, step_size ** 2, pyopencl.mem_flags.READ_WRITE
    )
    buffer2 = codecad.cl_util.Buffer(
        pyopencl.cltypes.float, step_size, pyopencl.mem_flags.READ_WRITE
    )

    ev_b1 = buffer1.enqueue_write(numpy.arange(buffer1.nitems, dtype=buffer1.dtype))
    ev_b2 = codecad.cl_util.opencl_manager.k.sum_tester(
        (step_size ** 2,), (step_size,), buffer1, buffer2, wait_for=[ev_b1]
    )
    ev_b1 = codecad.cl_util.opencl_manager.k.sum_tester(
        (step_size,), (step_size,), buffer2, buffer1, wait_for=[ev_b2]
    )
    ...
    assert (
        buffer2.array
        == [((2 * i + 1) * step_size - 1) * step_size / 2 for i in range(step_size)]
    ).all()
    ev_b1.wait()
...
def test_indexing_prefix_sum():
    ...
    buffer1 = codecad.cl_util.Buffer(
        pyopencl.cltypes.uint, step_size ** 2, pyopencl.mem_flags.READ_WRITE
    )
    buffer2 = codecad.cl_util.Buffer(
        pyopencl.cltypes.uint, step_size, pyopencl.mem_flags.READ_WRITE
    )

    counts = numpy.arange(buffer1.nitems, dtype=buffer1.dtype) % 5

    ev = buffer1.enqueue_write(counts)
    ev = codecad.cl_util.opencl_manager.k.indexing_sum_tester1(
        (step_size ** 2,), (step_size,), buffer1, buffer2, wait_for=[ev]
    )
    ev = codecad.cl_util.opencl_manager.k.indexing_sum_tester2(
        (step_size,), (step_size,), buffer1, buffer2, wait_for=[ev]
    )
    ev = codecad.cl_util.opencl_manager.k.indexing_sum_tester3(
        (step_size ** 2,), (step_size,), buffer1, buffer2, wait_for=[ev]
    )
